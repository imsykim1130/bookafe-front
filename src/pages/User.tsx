...
function logoutClickHandler(navigate: (path: string) => void, removeJwt: () => void) {
  removeJwt();
  navigate('/auth/sign-in');
}

function orderStatusClickHandler(navigate: (path: string) => void) {
  navigate('/admin/order-status');
}

function recommendBookClickHandler(navigate: (path: string) => void) {
  navigate('/admin/recommend-book');
}

const User = () => {
  const { email, profileImg, createDate, role } = useSelector((state: { user: userState }) => state.user);
      ...
      <div>
        ...
        <section className={'mt-[60px] flex flex-col gap-[15px] items-center text-md mx-[5%] md:mx-[10%] lg:mx-[15%]'}>
          ...
          <button
            ...
            onClick={() => logoutClickHandler(navigate, removeJwt)}
          >
            로그아웃
          </button>
        </section>
  ...
  const AdminRender = () => {
    return (
      <div className={'mt-[60px] flex flex-col gap-[15px] items-center text-md mx-[5%] md:mx-[10%] lg:mx-[15%]'}>
        <button
          className={
            'min-w-[300px] border-[1px] border-black border-opacity-40 rounded-[5px] py-[15px] duration-300 hover:bg-black hover:bg-opacity-10'
          }
          onClick={() => orderStatusClickHandler(navigate)}
        >
          {'배송상태 관리'}
        </button>
        <button
        ...
        <button
          ...
          onClick={() => {
            recommendBookClickHandler(navigate);
          }}
        >
          {'추천 책 관리'}
        </button>
